...
;(function($, window, document, undefined) {
	...
	methods = {
		...
		open: function (o,T) {
			...
			//显示弹框
			$B.stop().append(overlayTpl).find($o).css({'background-color':o.boxBg,'position':'fixed','top':t,'left':l,'z-index':999999,'transform':'translateY(-50%) translateX(-50%)','width':o.width,'height':o.height}).removeAttr('class').addClass('animated '+T.attr("class")).prepend(headTpl+iframeTpl).show();
	...
	*/
	$.extend({  
        ...
        /**
		 * @decription 返回顶部
		 * @param b : string 和当前元素底部的距离
		 * @param r : string 和当前元素右侧的距离
		 * @param obj : string 当前元素
		 * @param pos : string 回到顶部按钮的定位方式
		 */
        goTop: function (obj,pos,b,r) {
				obj= obj != undefined ? obj:$B;
				pos= pos != undefined ? pos:'fixed';
	        	b = b != undefined ? b : '100px';
				r = r != undefined ? r : '42px';
	        	methods.remove('#HGoTop');
	        	obj.stop().append('<a id="HGoTop" href="javascript:;" style="width:40px;height:40px;line-height:40px;border: 1px solid #DAD3D3;border-radius:50%;display:inline-block;text-align:center;background:rgba(255,255,255,.8);color:#666;position:'+pos+';bottom:'+b+';right:'+r+';z-index:10;"><i style="font-size: 20px;" class="iconfont icon-top"></i></a>').find('#HGoTop').hide();
	        	$T = $('#HGoTop');
	        	$W.on('scroll',function(){
	        		$W.scrollTop() > 300 ? $T.removeAttr('class').addClass('animated fadeIn').show() : $T.removeAttr('class').addClass('animated fadeOut');
	        	});
	        	$T.on('click',function(){ 
	        		$('body,html').animate({ scrollTop:0 },500); return false; //返回顶部按钮点击事件
	        	});
        },
		reTurn: function (obj,pos,b,l) {
			obj= obj != undefined ? obj:$B;
			pos= pos != undefined ? pos:'fixed';
			b = b != undefined ? b : '100px';
			l = l != undefined ? l : '42px';
			methods.remove('#reTurn');
			obj.stop().append('<a id="reTurn" target="_blank" href="suggestion.html" style="width:60px;height:69px;border-radius:50%;display:block;text-align:center;position:'+pos+';bottom:'+b+';left:'+l+';z-index:100000;"><img src="/public/app/site/statics/images/ichuang/suggestion.png"></a>').find('#reTurn').hide();
			$R = $('#reTurn');
			$W.on('scroll',function(){
				$W.scrollTop() > 300 ? $R.removeAttr('class').addClass('animated fadeIn').show() : $R.removeAttr('class').addClass('animated fadeOut');
			});
		},
		toHome: function (obj,pos,b,l) {
			obj= obj != undefined ? obj:$B;
			pos= pos != undefined ? pos:'fixed';
			b = b != undefined ? b : '100px';
			l = l != undefined ? l : '42px';
			// methods.remove('#toHome');
			obj.stop().append('<a id="toHome" href="index.html" style="display: none;width:40px;height:40px;border: 1px solid #eee;line-height:40px;border-radius:50%;text-align:center;background:rgba(255,255,255,.8);color:#666;position:'+pos+';bottom:'+b+';right:'+l+';z-index:100000;"><i style="font-size: 20px;" class="iconfont icon-home"></i></a>');
			// $R = $('#toHome');
			// $W.on('scroll',function(){
			// 	$W.scrollTop() > 300 ? $R.removeAttr('class').addClass('animated fadeIn').show() : $R.removeAttr('class').addClass('animated fadeOut');
			// });
		 ...
		 * @param callback : 回调函数
		 */
        dialog: function (type, title, content, time, callback,w) {
        		var t, tpl = '', footerTpl = '', isLock = false, okText = '确定', cancelText = '取消', width = 300, margin = '0 0 0 '+parseInt(-(width/2))+'px', type = type != undefined ? type : 'alert', time = time != undefined ? parseInt(time) : 0,
        		headerTpl = '<div id="hDialog-header" style="padding:10px;font-size:20px;color:#009CEA;text-align: center;position: relative">' + title +'' +
					'<i class="iconfont icon-quxiao" id="colseX" style="position: absolute;right: 10px;top: 10px;;cursor: pointer;color: #c4c4c4;font-size: 20px;"></i></div>',
    			okTpl = '<a href="javascript:;" style="padding:10px 40px;display:inline-block;border-radius:3px;background-color:#009CEA;color:#ffffff;" id="h_ok">'+ okText +'</a>',
    			cancelTpl = '<a href="javascript:;" style="padding:10px 40px;display:inline-block;border-radius:3px;margin-left:10px;background-color:#eee;color:#333;" id="h_cancel">'+ cancelText +'</a>',
    			contentTpl = '<div id="hDialog-content" style="padding:25px 15px;text-align:center;font-size: 16px;">' + content + '</div>',
    			maskTpl = '<div id="hDialog-mask" onclick="" style="width:100%;height:100%;background-color:rgba(0,0,0,0.6);position:fixed;top:0;left:0;z-index:99999;"></div>';
				if(type === 'alert'){ footerTpl = '<div id="hDialog-footer" style="padding:30px;text-align:center;font-size: 16px;">'+ okTpl +'</div>'; }
    			if(type === 'confirm'){ footerTpl = '<div id="hDialog-footer" style="padding:30px;text-align:right;font-size: 16px;">'+ okTpl + cancelTpl +'</div>'; }
    			if(!isLock) tpl += maskTpl;
    			tpl += '<div id="hDialog-wrap" class="animated fadeIn" style="width:'+width+'px;border-radius:5px;box-shadow:4px 4px 10px #666;-webkit-box-shadow:4px 4px 10px #666;background-color:#ffffff;border:1px solid #eee;position:fixed;top:210px;left:50%;margin:'+margin+';z-index:100000;">' + headerTpl + contentTpl + footerTpl + '</div>';
				...
				$("#hDialog-mask").on('click',function () { callback != undefined && methods.fire.call(this, callback);});
				$("#colseX").on('click',function () { callback != undefined && methods.fire.call(this, callback);})
        },
        /**
		 * @decription 关闭消息框
		 * @param callback : 回调函数
