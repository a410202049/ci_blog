...
class Article extends Home_Controller {
	public function __construct(){
		...
		$ym = $this->ArticleModel->article_filed();//年月归档
		$this->twig->assign('ym',$ym);
	}

	public function articleList($page_index = 1,$cid = null ){
		$articles = $this->ArticleModel->gets_by_article($page_index,15);
		$data['articles'] = $articles;
		$hots = $this->ArticleModel->gets_by_article('1','5','hot');
		$data['hots'] = $hots;
	...
	public function articleDate($date,$page_index = 1){
		$articles = $this->ArticleModel->gets_by_article($page_index,15,'date',$date);
		$data['articles'] = $articles;
		$hots = $this->ArticleModel->gets_by_article('1','5','hot');
		$data['hots'] = $hots;
		$data['page_index'] = $page_index;
	...
	public function articleTags($tid,$page_index = 1){
		$articles = $this->ArticleModel->gets_by_article($page_index,15,'tags',$tid);
		$data['articles'] = $articles;
		$hots = $this->ArticleModel->gets_by_article('1','5','hot');
		$data['hots'] = $hots;
		$data['page_index'] = $page_index;
	...
	public function detail($article_id){
		$data = $this->ArticleModel->getArticle($article_id);
		$this->ArticleModel->add_hit_num($article_id);
		$arr['article'] = $data;
		$rands = $this->ArticleModel->gets_by_article('1','5','rand');
		$arr['rands'] = $rands;
		$arr['about'] = $this->ArticleModel->about_articles($article_id);
		$arr['prev_next'] = $this->ArticleModel->article_prev_next($article_id);
		$arr['comments'] = $this->ArticleModel->get_article_comments($article_id);
	 	$this->twig->render('Article/detail',$arr);
	}

	public function search($page_index = 1){
		$keyword = $this->input->get('keyword');
		$articles = $this->ArticleModel->gets_by_article($page_index,2,'search',$keyword);
		$data['articles'] = $articles;
		$hots = $this->ArticleModel->gets_by_article('1','5','hot');
		$data['hots'] = $hots;
		$data['page_index'] = $page_index;
	...
	public function articleCategory($cid,$page_index = 1){
		$articles = $this->ArticleModel->gets_by_article($page_index,15,'category',$cid);
		$data['articles'] = $articles;
		$hots = $this->ArticleModel->gets_by_article('1','5','hot');
		$data['hots'] = $hots;
		$data['page_index'] = $page_index;
